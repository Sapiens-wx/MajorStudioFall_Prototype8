Shader "Hidden/Outline/Step4_ViewDirThreshold"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // View direction in view space ~ (0,0,1) or (0,0,-1).
                // We'll use +Z and take abs() of the dot so sign doesn't matter.
                float3 viewDir = float3(0.0, 0.0, 1.0);

                // Use one of the samples as representative normal
                float nDotV = abs(dot(normalize(nBL), viewDir)); // 1 when facing camera, 0 when grazing

                // Map to 0..1 such that grazing angles (small nDotV) increase threshold
